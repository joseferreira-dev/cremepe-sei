...
export default function Dashboard({ navigateTo }: Props) {
  ...
  const loadData = useCallback(() => {
    Promise.all([
      listProcesses({ page: 1, limit: 5 }),
      listProcesses({ page: 1, limit: 1, resumo: '1' }),
      listProcesses({ page: 1, limit: 1, status: 'em_andamento' }),
      listProcesses({ page: 1, limit: 1, status: 'finalizado' }),
      listProcesses({ page: 1, limit: 50, status: 'em_andamento' }),
    ]).then(([recent, comResumo, emAndamento, finalizados, emAndamentoList]) => {
      setProcesses(recent.processes);
      setEmAndamentoProcesses(emAndamentoList.processes);
      setCounts({
        total: recent.total,
        comResumo: comResumo.total,
  ...
  return (
    <div className="p-8 space-y-8" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Processos por Unidade (somente Em Andamento) */}
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          ...
          <p className="text-xs text-gray-400 mb-4">Apenas processos em andamento</p>
          {byUnit.length > 0 ? (
