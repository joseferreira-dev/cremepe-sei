  ...
  const handleImport = async () => {
    ...
    if (mode === 'text') {
      ...
      nums = content.split(/[\r\n,;]/).map((l) => l.trim()).filter(Boolean)
        .filter((l) => !l.toLowerCase().startsWith('numero'));
    }
    ...
    const res = await batchImport(nums);
    let done = 0;
    const interval = setInterval(() => {
      done = Math.min(nums.length, done + 1);
      setProgress(Math.round((done / nums.length) * 100));
      if (done >= nums.length) clearInterval(interval);
    }, 150);
