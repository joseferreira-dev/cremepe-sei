...
export default function Reports() {
  ...
  const byUnit = Object.entries(
    processes.reduce<Record<string, number>>((acc, p) => {
      acc[p.unidadeAtual.sigla || 'â€”'] = (acc[p.unidadeAtual.sigla || 'â€”'] || 0) + 1;
      return acc;
    }, {})
  ...
  return (
    <div className="p-8 space-y-8" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: "'Outfit', sans-serif" }}>RelatÃ³rios</h1>
          <p className="text-gray-500 text-sm mt-1">AnÃ¡lise consolidada dos processos gerenciados</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total de Processos', value: processes.length, color: '#009C60' },
          { label: 'Com Resumo IA', value: withResume, color: '#29ABE2' },
          { label: 'Em AnÃ¡lise', value: processes.filter((p) => p.status === 'em_analise').length, color: '#6366F1' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <h2 className="text-sm font-semibold text-gray-800 mb-5" style={{ fontFamily: "'Outfit', sans-serif" }}>DistribuiÃ§Ã£o por Status</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {byStatus.map((s) => {
              const count = s.value;
              const resumeCount = processes.filter((p) => STATUS_LABELS[p.status] === s.name && p.resumoIa).length;
              return (
                <tr key={s.name} className="border-b border-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {((count / processes.length) * 100).toFixed(0)}%
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
